...
- **网页翻译**：一键翻译当前网页，支持全文替换、原文对照和还原原始网页。
- **多 LLM 提供商**：内置 DeepSeek、阿里云百炼、智谱 AI、OpenAI 兼容接口和 Ollama。
- **可配置触发时机**：支持“点击图标时翻译”“选中后立即翻译”和“不翻译”三种模式。
...
| **模型** | 选择使用的模型 |
| **目标语言** | 全局配置，翻译目标语言，默认为“中文” |
| **翻译触发时机** | 全局配置，点击图标翻译 / 选中后立即翻译 / 不翻译 |
| **主题** | 全局配置，跟随系统 / 浅色 / 深色 |
...
1. 在任意网页中选中一段文本。
2. 根据触发模式点击翻译图标，或等待翻译结果自动弹出；选择“不翻译”时，划词不会触发翻译请求。
3. 在翻译气泡中查看结果，也可复制译文。
